...
__all__ = ["find_snd_data_start", "parse_snd_text", "parse_snd_file", "parse_snd_with_events", "parse_snd_header", "parse_snd_full"]

# SND method code -> readable name
METHOD_CODE_NAMES = {
    "25": "Totalsondering",
    "26": "Fjellkontrollboring",
    "23": "Dreietrykksondering",
    "21": "Dreiesondering",
    "22": "Enkel sondering",
    "7": "CPT",
}
...
def parse_snd_header(text: str) -> Dict:
    """
    Extract header info from SND text: coordinates, method code, date.

    SND file structure (first lines):
        line 0: Y-coordinate (northing)
        line 1: X-coordinate (easting)
        line 2: Z-coordinate (elevation / terrengkvote)
        ...
        After 3rd '*': method_code  date
    """
    lines = text.splitlines()
    if len(lines) < 4:
        raise ValueError("SND-fil for kort til å lese header.")

    y_coord = float(lines[0].strip())
    x_coord = float(lines[1].strip())
    z_coord = float(lines[2].strip())
    ...
    for i, ln in enumerate(lines):
        if ln.strip() == '*':
            star_count += 1
            if star_count == 3 and i + 1 < len(lines):
                parts = lines[i + 1].split()
                if parts:
                    method_code = parts[0]
                    method_name = METHOD_CODE_NAMES.get(method_code, method_code)
                if len(parts) >= 2:
                    date_str = parts[1]
                break
...
def parse_snd_full(text: str) -> Dict:
    """
    Parse SND text returning header (coordinates/method) + sounding data + events.
    """
    header = parse_snd_header(text)
    sounding = parse_snd_with_events(text)
    return {**header, **sounding}
