# PLAXIS 2D Python Scripting - Agent Quick Reference

Complete reference for 243 Plaxis 2D scripting commands.
For full examples with helper functions, see `plaxis_2d_commands.md`.

## Connection
```python
from plxscripting.easy import new_server
s_i, g_i = new_server('localhost', 10000, password='pw')  # Input
s_o, g_o = new_server('localhost', 10001, password='pw')  # Output
```

## Common Helper Functions
Many notebooks use these standard helper functions for setup:
- `create_geometry(s_i, g_i)` — Creates soil polygon, plate, line load with dynamic multiplier
- `simple_test_case(s_i, g_i)` — Creates geometry, meshes, returns (s_o, g_o)
- `dynamic_test_case(s_i, g_i)` — Creates geometry with dynamic phase
- `embeddedbeam_test_case(s_i, g_i)` — Creates geometry with embedded beam
- `suction_test_case(s_i, g_i)` — Creates geometry with GW flow BCs

## Key Object References
| Object | Description |
|--------|-------------|
| `g_i` | Input global environment |
| `s_i` | Input application server |
| `g_o` | Output global environment |
| `s_o` | Output application server |
| `g_i.Phases` | List of all phases |
| `g_i.InitialPhase` | The initial phase |
| `g_i.Soils` | List of all soil layers |
| `g_i.Plates` | List of all plates |
| `g_i.Lines` | List of all lines |
| `g_i.Points` | List of all points |
| `g_i.EmbeddedBeamRows` | List of all embedded beams |
| `g_i.LineLoads` | List of all line loads |
| `g_i.PointLoads` | List of all point loads |
| `g_i.Materials` | List of all materials |
| `g_o.ResultTypes` | Available result types |
| `g_o.Phases` | Output phases for results |
| `g_o.Plots` | Output plots list |
| `g_o.CenterLines` | Output centerlines |

## All Result Types (g_o.ResultTypes)
| Category | Type | Description |
|----------|------|-------------|
| Soil | `.Soil.Ux` | Horizontal displacement |
| Soil | `.Soil.Uy` | Vertical displacement |
| Soil | `.Soil.Utot` | Total displacement |
| Soil | `.Soil.Suction` | Soil suction |
| Soil | `.Soil.SigmaXX` | Stress XX |
| Soil | `.Soil.SigmaYY` | Stress YY |
| Soil | `.Soil.SigmaXY` | Stress XY |
| EmbeddedBeamRow | `.EmbeddedBeamRow.Utot` | Beam total displacement |
| CenterLine | `.CenterLine.M2D` | Bending moment |
...
### Input Commands (198)
`activate`, `add`, `adddesignapproachmateriallink`, `addedmass`, `addpoint`, `addsubcurve`, `addwaterpoint`, `allocmem`, `anchormat`, `apply`, `arrayp`, `arrayr`, `borehole`, `calculate`, `checkcalculationconditions`, `checkgeometry`, `clear`, `clearmaterial`, `close`, `coarsen`, `commands`, `connection`, `contraction`, `copylayers`, `count`, `createreachedwl`, `createreachedwlandcontinue`, `cutpoly`, `deactivate`, `delete`, `deletepoint`, `delruntimetoggle`, `designapproach`, `dischargefunction`, `displmultiplier`, `drain`, `dump`, `dumpboreholes`, `dumpcutobjects`, `dumpfixedendanchors`, `dumpgeogrids`, `dumpgroups`, `dumplinedispls`, `dumplineloads`, `dumplines`, `dumpmaterials`, `dumpmeshes`, `dumpn2nanchors`, `dumpnegativeinterfaces`, `dumpphases`, `dumppiles`, `dumpplates`, `dumppointdispls`, `dumppointloads`, `dumppoints`, `dumppolygons`, `dumppositiveinterfaces`, `dumpsoillayers`, `duplicate`, `echo`, `echotunnelvalidation`, `embeddedbeammat`, `embeddedbeamrow`, `embeddedpilemat`, `embeddedpilerow`, `export`, `extendtosymmetryaxis`, `fieldstress`, `filter`, `findcutobject`, `fixedendanchor`, `generatefromfielddata`, `generateintersectionpoints`, `generatethicklining`, `generatetunnel`, `geogrid`, `geogridmat`, `getcurveresults`, `getnormal`, `getresults`, `getsoillayerlevel`, `getsoillayerporepressure`, `gettoggle`, `gotoflow`, `gotomesh`, `gotosoil`, `gotostages`, `gotostructures`, `gotowater`, `group`, `groupfiltered`, `gwfbc`, `headfunction`, `heatfluxfunction`, `heattotalfluxfunction`, `help`, `import`, `importcrosssection`, `importfielddata`, `info`, `initializerectangular`, `insert`, `insertphase`, `insertpoint`, `insertsoillayer`, `insertsubcurve`, `insertwaterpoint`, `intersectsegments`, `invertdirection`, `kill`, `line`, `lineangles`, `linedispl`, `lineload`, `lineparallel`, `linerelative`, `linevector`, `loadfactorlabel`, `loadmultiplier`, `materialcommand`, `materialfactorlabel`, `mergeequivalents`, `mesh`, `meshd`, `move`, `movedisconnected`, `movepoint`, `movepointmagnetic`, `multiply`, `n2nanchor`, `neginterface`, `phase`, `plate`, `platemat`, `point`, `pointdispl`, `pointload`, `polycurve`, `polygon`, `posinterface`, `predict`, `preview`, `raise`, `raiseasync`, `raisethreaded`, `rectangle`, `redo`, `refine`, `regenerate`, `reinforcement`, `removeintermediatesteps`, `rename`, `reportmem`, `reset`, `resetlocal`, `retrievesuggestedparameters`, `rockboltsperpendicular`, `save`, `selectmeshpoints`, `set`, `setcolour`, `setcurrentphase`, `setdefaultmaterial`, `setglobalwaterlevel`, `setmaterial`, `setphysicalcpucount`, `setproperties`, `setsoillayerlevel`, `setsoillayerporepressure`, `settoggle`, `setundostacksize`, `setwaterdry`, `setwaterinterpolate`, `setwaterlevel`, `sleep`, `snap`, `snaplinear`, `soillayer`, `soillayerheight`, `soilmat`, `symmetricclose`, `tabulate`, `temperaturefunction`, `testasync`, `tfbc`, `tracepoly`, `transferfunction`, `transform`, `translateline`, `translatelinemagnetic`, `tunnel`, `undo`, `ungroup`, `view`, `viewmesh`, `waterlevel`, `well`, `writephasestomesh`

### Output Commands (45)
`add`, `addcurvepoint`, `allocmem`, `apply`, `centerline`, `centerlineconfig`, `clearcurvepoints`, `close`, `commands`, `count`, `delete`, `dump`, `dumpdisplacements`, `dumpfixities`, `dumpfrostlines`, `dumpphreaticlevels`, `dumpwaterloads`, `echo`, `export`, `filter`, `generate`, `getcurveresults`, `getcurveresultspath`, `getresults`, `getsingleresult`, `hide`, `index`, `info`, `linecrosssectionplot`, `raise`, `raiseasync`, `raisethreaded`, `rename`, `reportmem`, `set`, `setphysicalcpucount`, `setproperties`, `show`, `sleep`, `slice`, `structuralforcesplot`, `structureplot`, `tabulate`, `testasync`, `update`
...
## g_i.adddesignapproachmateriallink
...
## g_i.checkcalculationconditions
...
## g_i.createreachedwlandcontinue
...
## g_i.generateintersectionpoints
...
## g_i.retrievesuggestedparameters
